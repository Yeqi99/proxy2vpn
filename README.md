...
你已经有 HTTP/SOCKS5 代理，也有一台常开的电脑，但家里的原厂路由器只能配置 VPN。Proxy2VPN 在电脑上提供路由器能连接的入口，让你可以在路由器里为指定设备分流。

- **已有代理继续使用**：无需在这里导入订阅，也不提供代理节点。
- **无需刷路由器固件**：前提是原厂固件支持对应 VPN 客户端和所需分流功能。
- **网页完成设置**：中英文控制台、协议选择、代理测试、快捷方式和可关闭的登录自启。

**当前正在招募兼容性测试者**，尤其是 Apple Silicon Mac 和不同原厂路由器。Windows 已有本地验证；Mac 与具体头显/游戏体验仍需实测。欢迎[提交测试反馈](https://github.com/Yeqi99/proxy2vpn/issues/new?template=compatibility.yml)。
...
2. 自动打开的控制台位于 `http://127.0.0.1:18990/`。填写电脑内网 IP、路由器 IP、已有 HTTP/SOCKS5 代理地址和端口，选择接入口，点击「应用配置」。
...
- 网页可设置上游地址、端口、账号、接入协议与端口；启停、测试当前填写的代理、修改登录自启、显示账号、下载 WireGuard 配置。
